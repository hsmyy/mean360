...
function normalizeTickInterval(interval, multiples, magnitude, options) {
	...
	for (i = 0; i < multiples.length; i=i+1) {
...
SVGElement.prototype = {
	...
	*/
	...
	destroy: function () {
		...
		if (wrapper.stops) {
			for (i = 0; i < wrapper.stops.length; i=i+1) {
...
if (useCanVG) {
	...
	CanVGController = (function () {
		...
		function drawDeferred() {
			...
			for (callIndex = 0; callIndex < callLength; callIndex = callIndex + 1) {
		...
		if (hasData || axis.isLinked) {
			...
			if (axis.horiz && !axis.staggerLines && maxStaggerLines && !labelOptions.rotation) {
				...
				while (autoStaggerLines < maxStaggerLines) {
					...
					for (i = 0; i < sortedPositions.length; i = i + 1) {
					...
					if (overlap) {
						autoStaggerLines = autoStaggerLines + 1;
					} else {
						break;
					}
				}
...
Axis.prototype.getLogTickPositions = function (interval, min, max, minor) {
	...
	// 1, 2, 4, 6, 8, 10, 20, 40 etc. 
		...
		for (i = roundedMin; i < max + 1 && !break2; i = i + 1) {
			len = intermediate.length;
			for (j = 0; j < len && !break2; j = j + 1) {
...
Point.prototype = {
	...
	init: function (series, options, x) {
		...
		if (series.options.colorByPoint) {
			...
			series.colorCounter = series.colorCounter + 1;
			// loop back to zero
		...
		series.chart.pointCount = series.chart.pointCount + 1;
		return point;
	},
	/**
	...
	destroy: function () {
		...
		chart.pointCount = chart.pointCount - 1;
...
Series.prototype = {
	...
	processData: function (force) {
		...
		for (i = processedXData.length - 1; i >= 0; i = i - 1) {
	...
	generatePoints: function () {
		...
		for (i = 0; i < processedDataLength; i = i + 1) {
			cursor = cropStart + i;
			if (!hasGroupedData) {
				if (data[cursor]) {
					point = data[cursor];
	...
	getAttribs: function () {
		...
		if (seriesOptions.marker) { // line, spline, area, areaspline, scatter
			...
			stateOptionsHover.color = stateOptionsHover.color ||
				new Color(stateOptionsHover.color || seriesColor)
...
var AreaSeries = extendClass(Series, {
		...
		if (this.options.stacking && !this.cropped) { // cropped causes artefacts in Stock, and perf issue
			...
			each(keys, function (x) {
				...
				// correctly.
					...
					// a value (#1991)
					for (i = series.index; i <= yAxis.series.length; i = i + 1) {
	...
	getSegmentPath: function (segment) {
		...
		if (options.stacking && !this.closedStacks) {
			...
			for (i = segment.length - 1; i >= 0; i = i - 1) {
			...
			if (area) { // update
				...
				series[areaKey] = series.chart.renderer.path(areaPath)
					.attr({
						fill: pick(
							prop[2],
							new Color(prop[1]).setOpacity(pick(options.fillOpacity, 0.75)).get()
						),
						zIndex: 0 // #1069
					}).add(series.group);
			}
		...
		while ((i = i - 1) >= 0) {
			...
			if (distanceOption > 0) {
				...
				for (j = 0; j < length; j = j + 1) {
					...
					for (slotI = 0; slotI < slotsLength; slotI = slotI + 1) {
...
var TrackerMixin = Highcharts.TrackerMixin = {
	...
	drawTrackerGraph: function () {
		...
		for (i = 0; i < singlePoints.length; i = i + 1) {
...
extend(Point.prototype, {
	...
	setState: function (state, move) {
		...
		if (haloOptions && haloOptions.size) {
			...
			halo.attr(extend({
				fill: new Color(point.color || series.color).setOpacity(haloOptions.opacity).get()
